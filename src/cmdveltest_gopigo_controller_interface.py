...
class motor_driver: 
    ...
    def callback_gpg(self,data):
        rospy.loginfo("Received a /gopigo_vel message!")
        mode = data.mode
        move = data.move

        if(mode=="cm"):
            self.gpg.drive_cm(move)
        elif(mode=="deg"):
            self.gpg.drive_degrees(move)
        else:
            rospy.loginfo("Messaggio in un formato non riconosciuto")

    def __init__(self):
        # motor setup
        self.gpg = easy.EasyGoPiGo3() # GoPiGo3 Motor
        self.gpg.set_robot_constants(WHEEL_RADIUS*2*1000, WHEEL_DISTANCE*1000)
        rospy.Subscriber("cmd_vel", Twist, self.callback, queue_size=1) # subscribe to cmd_vel topic
        rospy.Subscriber("gopigo_vel", Gopigo, self.callback_gpg, queue_size=1) # subscribe to cmd_vel topic
